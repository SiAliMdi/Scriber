...
import { Decision, SearchParameters, SearchResult } from "@/@types/search";
...
const search = async (searchParameters: SearchParameters,
                      setSearchResult: React.Dispatch<React.SetStateAction<SearchResult>>) : Promise<SearchResult> =>
                         {
  try {
    const searchResults = await TypesenseClient.collections(
      `${import.meta.env.VITE_TYPESENSE_COLLECTION_NAME}`).documents().search(searchParameters);
    
    const hits: Decision[] | []= searchResults.hits?.map(
      (hit:any) => { 
        const intDate = hit.document.j_date; // Example timestamp
        const date = new Date(intDate * 1000); // Convert to milliseconds
        const formattedDate = date.toISOString().split("T")[0];
        
        return {id: hit.document.id,
...
const logCollection = async () => {
  try {
    ...
    console.log("**** mycollection " + collection['name'] + collection['num_documents'] );
...
export { fetchCategories, fetchVilles, search, logCollection };
